...
if (config?.git?.deploymentEnabled === true && fs.existsSync(oneOffMarkerPath)) {
  console.log("[cost-guard] ONE-OFF: guarded playback hotfix deployment marker is present.");
  process.exit(0);
}

console.error("[cost-guard] BLOCKED: git.deploymentEnabled must stay false unless a supervised one-off marker is present.");
process.exit(1);
