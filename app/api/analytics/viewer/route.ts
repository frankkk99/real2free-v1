...
function requestDevice(userAgent: string): "mobile" | "tablet" | "desktop" | "unknown" {
...
function referrerHost(raw: unknown, request: NextRequest): string | null {
  if (typeof raw !== "string" || !raw || raw.length > 2048) return null;
  try {
    const url = new URL(raw);
    const hostname = url.hostname.toLowerCase().slice(0, 255);
    return hostname && hostname !== request.nextUrl.hostname.toLowerCase() ? hostname : null;
  } catch {
    return null;
  }
}
...
export async function POST(request: NextRequest) {
  ...
  if (!path?.startsWith("/") || path.startsWith("/api/") || path.startsWith("/admin")) {
    return NextResponse.json({ error: "invalid_path" }, { status: 400, headers: noStoreHeaders() });
  }
