...
function PosterCard({
  ...
  return (
    <article className={styles.card}>
      <button className={styles.posterButton} type="button" onClick={onOpen} aria-label={`ดูข้อมูล ${movie.thaiTitle}`}>
        <span className={styles.poster}>
          {movie.posterUrl && !imageFailed ? (
            <img src={movie.posterUrl} alt={movie.thaiTitle} loading="lazy" decoding="async" referrerPolicy="no-referrer" onError={() => setImageFailed(true)} />
          ) : (
      ...
      <button className={`${styles.cardFavorite} ${favorite ? styles.cardFavoriteActive : ""}`} type="button" onClick={onFavorite} aria-label={favorite ? "นำออกจากรายการโปรด" : "เพิ่มในรายการโปรด"}>
...
function DetailModal({
  ...
  const labels = [...new Set(movie.players.map((player) => player.label).filter(Boolean))].slice(0, 3);
        ...
        <div className={styles.modalGrid}>
          ...
          <div className={styles.modalBody}>
            ...
            <div className={styles.modalScore}>
              <span><Star fill="currentColor" /> <strong>{movie.rating ? movie.rating.toFixed(1) : "-"}</strong></span>
              <span>{movie.players.length > 1 ? `${movie.players.length} ตัวเลือกรับชม` : "พร้อมรับชม"}</span>
            </div>
            ...
            <div className={styles.modalActions}>
              <button className={styles.watchButton} type="button" onClick={onWatch}><Play fill="currentColor" /><span><strong>ไปหน้ารับชม</strong><small>ดูรายละเอียดและเลือกตัวรับชม</small></span><ChevronRight /></button>
              <button className={`${styles.favoriteAction} ${favorite ? styles.favoriteActionActive : ""}`} type="button" onClick={onFavorite}><Heart fill={favorite ? "currentColor" : "none"} /> {favorite ? "บันทึกแล้ว" : "เพิ่มรายการโปรด"}</button>
            </div>
          </div>
        </div>
      </section>
...
export default function MovieHomeV2() {
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => setQuery(cleanCatalogSearch(queryInput)), 320);
  ...
  const fetchCatalog = useCallback(async (targetPage: number, append: boolean) => {
    ...
    try {
      ...
      let builder = supabase.from("real2free_public_titles").select(PUBLIC_CATALOG_FIELDS, { count: "exact" });
      ...
      if (viewMode === "popular") builder = builder.order("rating", { ascending: false, nullsFirst: false }).order("vote_count", { ascending: false, nullsFirst: false });
      else if (viewMode === "new") builder = builder.order("release_date", { ascending: false, nullsFirst: false }).order("updated_at", { ascending: false });
      ...
      let mapped = ((data || []) as unknown as PublicCatalogRow[]).map(mapPublicCatalogRow).filter((item): item is PublicCatalogItem => Boolean(item));
      if (viewMode === "history") {
        const order = new Map(history.map((id, index): [string, number] => [id, index]));
  ...
  }, [favorites, genre, history, query, viewMode, year]);

  useEffect(() => { void fetchCatalog(0, false); }, [fetchCatalog]);
  ...
  const goWatch = (movie: PublicCatalogItem) => {
    const nextHistory = [movie.id, ...history.filter((id) => id !== movie.id)].slice(0, 100);
    ...
    <main className={styles.page}>
      <header className={styles.header}>
        ...
        <nav className={styles.nav} aria-label="เมนูหลัก">
          {mainNav.map((item) => <button key={item.mode} className={viewMode === item.mode ? styles.navActive : ""} type="button" onClick={() => chooseMode(item.mode)}>{item.label}</button>)}
        </nav>

        <div className={styles.headerActions}>
          <label className={styles.searchBox}><Search /><input value={queryInput} onChange={(event) => setQueryInput(event.target.value)} placeholder="ค้นหาหนังหรือซีรีส์" />{queryInput ? <button type="button" onClick={() => setQueryInput("")} aria-label="ล้างคำค้น"><X /></button> : null}</label>
      ...
      {heroMovie && viewMode === "home" && !query ? (
        <section className={styles.hero}>
          {heroMovie.backdropUrl ? <img src={heroMovie.backdropUrl} alt="" referrerPolicy="no-referrer" /> : null}
          <span className={styles.heroShade} />
          <div className={styles.heroContent}>
            <span className={styles.heroEyebrow}>เรื่องเด่นสำหรับคุณ</span>
            <h1>{heroMovie.thaiTitle}</h1>
            {heroMovie.title !== heroMovie.thaiTitle ? <h2>{heroMovie.title}</h2> : null}
            <div className={styles.heroMeta}><span><Star fill="currentColor" /> {heroMovie.rating ? heroMovie.rating.toFixed(1) : "-"}</span><span>{heroMovie.year || ""}</span><span>{runtimeLabel(heroMovie.runtime)}</span><span>{heroMovie.genres.slice(0, 2).join(" • ")}</span></div>
            <p>{heroMovie.overview || "เลือกเรื่องที่ชอบแล้วไปยังหน้ารับชมได้ทันที"}</p>
            <div className={styles.heroActions}><button type="button" onClick={() => goWatch(heroMovie)}><Play fill="currentColor" /> รับชมตอนนี้</button><button type="button" onClick={() => setSelectedMovie(heroMovie)}>ดูรายละเอียด</button></div>
          </div>
          {heroItems.length > 1 ? (
            <>
              ...
              <div className={styles.heroDots}>{heroItems.map((item, index) => <button key={item.id} className={index === heroIndex ? styles.heroDotActive : ""} type="button" onClick={() => setHeroIndex(index)} aria-label={`เรื่องที่ ${index + 1}`} />)}</div>
            </>
          ) : null}
        </section>
      ) : null}
      ...
      <section className={styles.quickBar} aria-label="ทางลัด">
        {[...mainNav.slice(1), { mode: "favorites" as ViewMode, label: "รายการโปรด", icon: Bookmark }, { mode: "history" as ViewMode, label: "ดูล่าสุด", icon: History }].map((item) => {
          ...
          return <button key={item.mode} className={viewMode === item.mode ? styles.quickActive : ""} type="button" onClick={() => chooseMode(item.mode)}><Icon /><span>{item.label}</span></button>;
        })}
      </section>

      <section id="catalog" className={styles.catalog}>
        <div className={styles.catalogHeading}>
          <div><span>{viewLabels[viewMode]}</span><h2>{loading && !items.length ? "กำลังเตรียมรายการ" : `${total.toLocaleString("th-TH")} รายการ`}</h2></div>
          <div className={styles.activeFilters}>{genre !== "ทั้งหมด" ? <button type="button" onClick={() => setGenre("ทั้งหมด")}>{genreFilters.find((item) => item.value === genre)?.label}<X /></button> : null}{year !== "ทั้งหมด" ? <button type="button" onClick={() => setYear("ทั้งหมด")}>{year}<X /></button> : null}{hasFilters ? <button className={styles.clearButton} type="button" onClick={clearAll}>ล้างทั้งหมด</button> : null}</div>
        </div>

        {loadFailed && !items.length ? (
          <div className={styles.empty}><RotateCcw /><h3>ยังเปิดรายการไม่ได้</h3><p>กรุณาลองอีกครั้ง</p><button type="button" onClick={() => void fetchCatalog(0, false)}>ลองใหม่</button></div>
        ) : loading && !items.length ? (
          <div className={styles.loading}><LoaderCircle /><span>กำลังเตรียมรายการ...</span></div>
        ) : !items.length ? (
          ...
          <>
            <div className={styles.sectionTitle}><div><Clock3 /><span><strong>มาใหม่</strong><small>รายการที่เพิ่งอัปเดต</small></span></div><button type="button" onClick={() => chooseMode("new")}>ดูทั้งหมด <ChevronRight /></button></div>
            <div className={styles.grid}>{freshItems.map((movie) => <PosterCard key={movie.id} movie={movie} favorite={favorites.has(movie.id)} onOpen={() => setSelectedMovie(movie)} onPlay={() => goWatch(movie)} onFavorite={() => toggleFavorite(movie.id)} />)}</div>

            <div className={styles.sectionTitle}><div><Flame /><span><strong>น่าดูตอนนี้</strong><small>คะแนนดีและได้รับความนิยม</small></span></div><button type="button" onClick={() => chooseMode("popular")}>ดูทั้งหมด <ChevronRight /></button></div>
            <div className={styles.grid}>{popularItems.map((movie, index) => <PosterCard key={movie.id} movie={movie} rank={index + 1} favorite={favorites.has(movie.id)} onOpen={() => setSelectedMovie(movie)} onPlay={() => goWatch(movie)} onFavorite={() => toggleFavorite(movie.id)} />)}</div>
          </>
        ) : (
          <div className={styles.grid}>{items.map((movie, index) => <PosterCard key={movie.id} movie={movie} rank={viewMode === "popular" ? index + 1 : undefined} favorite={favorites.has(movie.id)} onOpen={() => setSelectedMovie(movie)} onPlay={() => goWatch(movie)} onFavorite={() => toggleFavorite(movie.id)} />)}</div>
        )}

        {hasMore && items.length ? <button className={styles.loadMore} type="button" disabled={loadingMore} onClick={() => void fetchCatalog(page + 1, true)}>{loadingMore ? <><LoaderCircle /> กำลังโหลด...</> : "แสดงเพิ่มเติม"}</button> : null}
      </section>
      ...
      {filterOpen ? (
        <div className={styles.sheetBackdrop} role="presentation" onMouseDown={() => setFilterOpen(false)}>
          <aside className={styles.filterSheet} onMouseDown={(event) => event.stopPropagation()}>
            <div className={styles.sheetHeader}><div><SlidersHorizontal /><span><strong>ตัวกรอง</strong><small>เลือกได้ตามที่ต้องการ</small></span></div><button type="button" onClick={() => setFilterOpen(false)}><X /></button></div>
            <div className={styles.filterGroup}><strong>ประเภทเนื้อหา</strong><div>{[{ value: "home", label: "ทั้งหมด" }, { value: "movie", label: "ภาพยนตร์" }, { value: "series", label: "ซีรีส์" }, { value: "anime", label: "อนิเมะ" }].map((item) => <button key={item.value} className={viewMode === item.value || (item.value === "home" && !["movie", "series", "anime"].includes(viewMode)) ? styles.filterSelected : ""} type="button" onClick={() => setViewMode(item.value as ViewMode)}>{item.label}{viewMode === item.value ? <Check /> : null}</button>)}</div></div>
            <div className={styles.filterGroup}><strong>แนว</strong><div>{genreFilters.map((item) => <button key={item.value} className={genre === item.value ? styles.filterSelected : ""} type="button" onClick={() => setGenre(item.value)}>{item.label}{genre === item.value ? <Check /> : null}</button>)}</div></div>
            <div className={styles.filterGroup}><strong>ปีที่ฉาย</strong><div>{yearFilters.map((item) => <button key={item} className={year === item ? styles.filterSelected : ""} type="button" onClick={() => setYear(item)}>{item}{year === item ? <Check /> : null}</button>)}</div></div>
      ...
      {mobileMenuOpen ? (
        <div className={styles.sheetBackdrop} role="presentation" onMouseDown={() => setMobileMenuOpen(false)}>
          <aside className={styles.mobileMenu} onMouseDown={(event) => event.stopPropagation()}>
            ...
            <nav>{[...mainNav, { mode: "favorites" as ViewMode, label: "รายการโปรด", icon: Bookmark }, { mode: "history" as ViewMode, label: "ดูล่าสุด", icon: History }].map((item) => { const Icon = item.icon; return <button key={item.mode} className={viewMode === item.mode ? styles.mobileMenuActive : ""} type="button" onClick={() => chooseMode(item.mode)}><Icon /><span>{item.label}</span><ChevronRight /></button>; })}</nav>
            <button className={styles.mobileTheme} type="button" onClick={toggleTheme}>{theme === "dark" ? <Sun /> : <Moon />}<span>{theme === "dark" ? "เปลี่ยนเป็นโหมดสว่าง" : "เปลี่ยนเป็นโหมดมืด"}</span></button>
          </aside>
        </div>
      ) : null}

      {selectedMovie ? <DetailModal movie={selectedMovie} favorite={favorites.has(selectedMovie.id)} onClose={() => setSelectedMovie(null)} onWatch={() => goWatch(selectedMovie)} onFavorite={() => toggleFavorite(selectedMovie.id)} /> : null}
    </main>
  );
}
